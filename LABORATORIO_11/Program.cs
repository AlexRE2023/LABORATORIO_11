...
namespace LABORATORIO_11
{
    class Program
    {
        static void Main()
        {
            string[] notas = new string[100]; // Un arreglo de cadenas para almacenar las notas (puedes ajustar el tamaño)
            int cantidadNotas = 0; // Variable para llevar un seguimiento de la cantidad de notas registradas
        ...
        static void RegistrarNotas(string[] notas, ref int cantidadNotas)
        {
            ...
            if (cantidadNotas < notas.Length)
            {
                ...
                if (int.TryParse(Console.ReadLine(), out int opcion))
                {
                    if (opcion == 1)
                    {
                        RegistrarNotas(notas, ref cantidadNotas); // Llamada recursiva para registrar otra nota
                    }
                }
                else if (opcion == 2)
                {
        ...
        static void ModificarNota(string[] notas, int cantidadNotas)
        {
            ...
            if (cantidadNotas > 0)
            {
                ...
                if (posicion >= 0 && posicion < cantidadNotas)
                {
                    ...
                    MostrarNotasConDestacado(notas, cantidadNotas, posicion); // Mostrar notas antes de la modificación
        ...
        static void MostrarNotasConDestacado(string[] notas, int cantidadNotas, int posicionDestacada)
        {
            for (int i = 0; i < cantidadNotas; i++)
            {
                ...
                {
                    Console.Write("[" + notas[i] + "]"); // Destacar la nota seleccionada
                }
                else
                {
                    Console.Write(notas[i]);
